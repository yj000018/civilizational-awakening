...
// Vite glob import — reads from content/{locale}/**/*.md
// Default locale is 'en'. Translated locales (fr, it) are generated by scripts/translate-site.ts
const allFiles = import.meta.glob('../content/en/**/*.md', {
  query: '?raw',
  import: 'default',
  eager: true,
}) as Record<string, string>;

// Translated files — loaded lazily per locale
...
function pathToTypeAndSlug(path: string): { type: string; slug: string } {
  // path like: ../content/en/pillars/spirit-soul.md or ../content/fr/pillars/spirit-soul.md
  const stripped = path.replace('../content/', '');
  const parts = stripped.split('/');
  // parts[0] = locale (en/fr/it), parts[1] = type dir, parts[2] = slug.md
  const typeDir = parts.length >= 3 ? parts[1] : parts[0];
  const slugFile = parts.length >= 3 ? parts[2] : parts[1];
  const type = typeDir.replace(/s$/, ''); // pillars → pillar
